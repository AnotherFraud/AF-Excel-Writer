...
public class WriteToExcelTemplateXLSXConfig {
    ...
    WriteToExcelTemplateXLSXConfig(final PortsConfiguration portsCfg) {
        m_srcFileChooser = new SettingsModelReaderFileChooser(CFG_SRC_FILE_CHOOSER, portsCfg,
        		WriteToExcelTemplateXLSXNodeFactory.FS_CONNECT_GRP_ID
        		,EnumConfig.create(FilterMode.FILE));

        m_destFileChooser = new SettingsModelWriterFileChooser(CFG_DEST_FILE_CHOOSER, portsCfg,
        		WriteToExcelTemplateXLSXNodeFactory.FS_CONNECT_GRP_ID, EnumConfig.create(FilterMode.FILE),
