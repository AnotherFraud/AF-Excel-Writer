...
public class WriteToExcelTemplateStreamingConfig {
    ...
    WriteToExcelTemplateStreamingConfig(final PortsConfiguration portsCfg) {
        m_srcFileChooser = new SettingsModelReaderFileChooser(CFG_SRC_FILE_CHOOSER, portsCfg,
        		WriteToExcelTemplateStreamingNodeFactory.FS_CONNECT_GRP_ID
        		,EnumConfig.create(FilterMode.FILE));

        m_destFileChooser = new SettingsModelWriterFileChooser(CFG_DEST_FILE_CHOOSER, portsCfg,
        		WriteToExcelTemplateStreamingNodeFactory.FS_CONNECT_GRP_ID, EnumConfig.create(FilterMode.FILE),
