...
public class WriteToExcelTemplateV2Config {
    ...
    WriteToExcelTemplateV2Config(final PortsConfiguration portsCfg) {
        m_srcFileChooser = new SettingsModelReaderFileChooser(CFG_SRC_FILE_CHOOSER, portsCfg,
        		WriteToExcelTemplateV2NodeFactory.FS_CONNECT_GRP_ID
        		,EnumConfig.create(FilterMode.FILE));

        m_destFileChooser = new SettingsModelWriterFileChooser(CFG_DEST_FILE_CHOOSER, portsCfg,
        		WriteToExcelTemplateV2NodeFactory.FS_CONNECT_GRP_ID, EnumConfig.create(FilterMode.FILE),
